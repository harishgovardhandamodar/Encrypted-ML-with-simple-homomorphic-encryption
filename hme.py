...
def encryption_train(X,y):
    # U1 is an orthogonal matrix (generate random matrices from the classical compact groups)
    U1 = ortho_group.rvs(dim=X.shape[0])

    # U2 is an invertible matrix (generate random matrices from the classical compact groups)
    if X.shape[1] > 1:
